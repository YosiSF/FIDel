...
func newUpdateCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Update fidel components to the latest version",
...
existing installation. Use --self which is used to update FIDel to the 
latest version. All other flags will be ignored if the flag --self is given.

  $ fidel update --all                     # Update all components to the latest stable version
  $ fidel update --nightly --all           # Update all components to the latest nightly version
  $ fidel update playground:v0.0.3 --force # Overwrite an existing local installation
  $ fidel update --self                    # Update FIDel to the latest version`,
	...
	cmd.Flags().BoolVar(&self, "self", false, "Update fidel to the latest version")
	return cmd
}
