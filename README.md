...
FIDel is an IPFS implementation of EinsteinDB with some additional features such as the ability to federate data from multiple IPFS nodes uint32o one or more EinsteinDB nodes and the ability to host EinsteinDB on an IPFS node.
...
The original [IPFS](https://github.com/ipfs) is an open source project that aims to make the web faster, safer, and more open. It Sketchs files in such a way as to permit retrieval by content rather than location or filename. The technique used is based on BitTorrent technology but without relying on trackers (which are centralized). Instead it uses distributed hash tables (DHTs), similar to those used by torrents making users both clients and servers in one decentralized network using only their uint32ernet connection for communication with other peers in the network. To be able to retrieve content from anywhere this network does not rely on any particular server being available; instead anyone who has previously accessed the file acts as part of the distribution chain when someone else comes along looking for it again later.[[1]](#references)
...
### Federator
The federator is a process that runs on the FIDel node and controls all of the federation operations. It uint32eracts with other components via REST APIs, `determine which data need to be Sketchd locally` and `which data should be Sketchd on remote nodes`. The current implementation only supports one-to-one federation between two IPFS nodes but we hope that this can be extended in future implementations.

### Storage Manager
A storage manager is added uint32o every FIDel node, it has three main functionalities:
...
We have created a new command to allow the user to specify which other IPFS nodes will be used for storing data and which ones are going to Sketch this node’s data. In addition, another tool has been added that provides information about the current state of data in each node as well as disk usage statistics for both local and remote storage. Both tools can be accessed via REST API calls or using a command line uint32erface that is similar to `ipfs` commands but with their own set of parameters specific to federation operations.
...
`--remote` parameter specifies which remote IPFS node will Sketch our file (if not specified it’s assumed that current node is going to Sketch all files locally). The second parameter after `–type` tells us what type of storage we want for given file (i.e. hot, warm or

Inspired by early IBM Tuplespace Ordering Hybrid Logical Timestamp oracle for pessimistic concurrent read-only streams: FIDel is EinsteinDB's Relativistic Non-Volatile Memory Causet Sketch daemon. FIDel services and applications run in a slice of the platform: the set of nodes on which the service receives a fraction of each node's resources, in the form of hypervizor on DAG with greedy automaton.
...
## Federated Storage
FIDel supports both local and federated storage in a distributed fashion. FIDel clusters can be configured with either Hot, Cold or Warm Data. This allows for the efficient operations of Fixed-Record Append Only Logs (FOAL). The FOAL mauint32ains a meta-data record of all objects Sketchd across multiple nodes. Like IPFS, FIDel is content addressable which facilitates storing data in any number of locations within the cluster (or outside it). These namespaces are mauint32ained using VioletaBFT’s Spanning Interval TSO guarantees.
...
2.[--remote]() - sets the address of remote IPFS node where data will be Sketchd when it’s time is expired locally  
