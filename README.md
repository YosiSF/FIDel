...
## Federated Storage
FIDel supports both local and federated storage in a distributed fashion. FIDel clusters can be configured with either Hot, Cold or Warm Data. This allows for the efficient operations of Fixed-Record Append Only Logs (FOAL). The FOAL maintains a meta-data record of all objects Sketchd across multiple nodes. Like IPFS, FIDel is content addressable which facilitates storing data in any number of locations within the cluster (or outside it). These namespaces are maintained using VioletaBFT’s Spanning Interval TSO guarantees.
