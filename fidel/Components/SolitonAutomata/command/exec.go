...
func newExecCmd() *cobra.Command {
	opt := solitonAutomata.ExecOptions{}
	cmd := &cobra.Command{
		Use:   "exec <solitonAutomata-name>",
		Short: "Run shell command on host in the milevadb solitonAutomata",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return manager.Exec(solitonAutomataName, opt, gOpt)
		},
	}

	cmd.Flags().StringVar(&opt.Command, "command", "ls", "the command run on solitonAutomata host")
