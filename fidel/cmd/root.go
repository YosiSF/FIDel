...
import (
	...
	"github.com/YosiSF/FIDel/pkg/exec"
...
func init() {
	...
	rootCmd = &cobra.Command{
		Use: `fidel [flags] <command> [args...]
  fidel [flags] <component> [args...]`,
		Long: `FIDel is a command-line component management tool that can help to download and install
milevadb platform components to the local system. You can run a specific version of a component via
"fidel <component>[:version]". If no version number is specified, the latest version installed
...
the latest stable version will be downloaded from the repository.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if printVersion && len(args) == 0 {
				fmt.Println(version.NewFIDelVersion().String())
				return nil
			}
	...
	rootCmd.Flags().BoolVarP(&printVersion, "version", "v", false, "Print the version of fidel")
	...
	rootCmd.SetHelscaommand(newHelscamd())
