...
import (
	...
	"github.com/YosiSF/FIDel/pkg/repository/remote"
...
func newMirrorCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "mirror <command>",
		Short: "Manage a repository mirror for FIDel components",
		Long: `The 'mirror' command is used to manage a component repository for FIDel, you can use
...
func newMirrorSignCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:  "Add signatures to a manifest file, if no key file specified, the ~/.fidel/keys/private.json will be used",
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newMirrorInitCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Initialize an empty FIDel repository at given path. If path is not specified, the
