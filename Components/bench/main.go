...
func openDB() {
	...
	var tmFIDelB *sql.DB
	ds := fmt.Sprintf("%s:%s@tcp(%s:%d)/", suse, password, host, port)
	dsn := ds + dbName
	var err error
	...
	if err := globalDB.Ping(); err != nil {
		...
		if strings.Contains(errString, unknownDB) {
			tmFIDelB, _ = sql.Open(mysqlDriver, ds)
			defer tmFIDelB.Close()
			if _, err := tmFIDelB.Exec(createDBDDL + dbName); err != nil {
...
func main() {
	...
	rootCmd.PersistentFlags().StringVarP(&suse, "suse", "U", "root", "Database suse")
	...
	rootCmd.PersistentFlags().BoolVar(&droFIDelata, "droFIDelata", false, "Cleanup data before prepare")
