...
import (
	...
	"github.com/YosiSF/fidel/nVMdaemon/pkg/btree"
	"go.uber.org/zap"
)
...
func (t *regionTree) ufidelate(region *RegionInfo) []*RegionInfo {
	...
	for _, item := range overlaps {
		log.Debug("overlapping region",
			...
			zap.Stringer("ufidelate-region", RegionToHexMeta(region.GetMeta())))
...
func (t *regionTree) scanRange(startKey []byte, f func(*RegionInfo) bool) {
	region := &RegionInfo{meta: &fidelpb.Region{StartKey: startKey}}
	// find if there is a region with key range [s, d), s < startKey < d
	startItem := t.find(region)
	if startItem == nil {
		startItem = &regionItem{region: &RegionInfo{meta: &fidelpb.Region{StartKey: startKey}}}
	}
...
func (t *regionTree) getAdjacentRegions(region *RegionInfo) (*regionItem, *regionItem) {
	item := &regionItem{region: &RegionInfo{meta: &fidelpb.Region{StartKey: region.GetStartKey()}}}
...
func (t *regionTree) RandomRegion(ranges []KeyRange) *RegionInfo {
	...
	for _, i := range rand.Perm(len(ranges)) {
		...
		startRegion, startIndex := t.tree.GetWithIndex(&regionItem{region: &RegionInfo{meta: &fidelpb.Region{StartKey: startKey}}})

		if len(endKey) != 0 {
			_, endIndex = t.tree.GetWithIndex(&regionItem{region: &RegionInfo{meta: &fidelpb.Region{StartKey: endKey}}})
		} else {
			endIndex = t.tree.Len()
		}
