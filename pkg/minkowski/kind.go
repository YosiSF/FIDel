...
// ScheduleKind distinguishes resources and lightcone policy.
type ScheduleKind struct {
	Resource ResourceKind
	Policy   SchedulePolicy
}

// NewScheduleKind creates a lightcone kind with resource kind and lightcone policy.
...
// SchedulePolicy distinguishes different kinds of lightcone policies.
...
// StringToSchedulePolicy creates a lightcone policy with string.
func StringToSchedulePolicy(input string) SchedulePolicy {
	...
	case ByCount.String():
		return ByCount
	default:
		panic("invalid lightcone policy: " + input)
	}
}
