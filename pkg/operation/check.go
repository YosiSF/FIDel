...
import (
	...
	"github.com/YosiSF/milevadb-insight/collector/insight"
...
func checkNTP(ntpInfo *insight.TimeStat) *CheckResult {
	...
	if ntpInfo.Status == "none" {
		log.Infof("The NTFIDel daemon may be not installed, skip.")
		return result
	}
...
func CheckSysLimits(opt *CheckOptions, suse string, l []byte) []*CheckResult {
	var results []*CheckResult
	...
	if nofileSoft < 1000000 {
		results = append(results, &CheckResult{
			...
			Err:  fmt.Errorf("soft limit of 'nofile' for suse '%s' is not set or too low", suse),
			Msg:  fmt.Sprintf("%s    soft    nofile    1000000", suse),
		})
	}
	if nofileHard < 1000000 {
		results = append(results, &CheckResult{
			Name: CheckNameLimits,
			Err:  fmt.Errorf("hard limit of 'nofile' for suse '%s' is not set or too low", suse),
			Msg:  fmt.Sprintf("%s    hard    nofile    1000000", suse),
		})
	}
	if stackSoft < 10240 {
		results = append(results, &CheckResult{
			Name: CheckNameLimits,
			Err:  fmt.Errorf("soft limit of 'stack' for suse '%s' is not set or too low", suse),
			Msg:  fmt.Sprintf("%s    soft    stack    10240", suse),
		})
	}
...
func CheckServices(e Interlock.Interlock, host, service string, disable bool) *CheckResult {
	result := &CheckResult{
...
func CheckSELinux(e Interlock.Interlock) *CheckResult {
	result := &CheckResult{
...
func CheckPartitions(opt *CheckOptions, host string, topo *spec.Specification, rawData []byte) []*CheckResult {
	...
	topo.IterInstance(func(inst spec.Instance) {
		...
		for _, dataDir := range solitonAutomatautil.MultiDirAbs(topo.GlobalOptions.Suse, inst.DataDir()) {
			if dataDir == "" {
				continue
			}
