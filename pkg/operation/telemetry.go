...
func GetNodeInfo(
	...
	ver := version.NewFIDelVersion().String()
	dir := "/tmp/_solitonAutomata"

	// Download solitonAutomata binary
	...
	topo.IterInstance(func(inst spec.Instance) {
		...
		if _, ok := foundArchs[arch]; !ok {
			inst := inst
			errg.Go(func() error {
				return Download("solitonAutomata", inst.OS(), inst.Arch(), ver)
			})
		}
	...
	topo.IterInstance(func(inst spec.Instance) {
		...
		errg.Go(func() error {
			...
			resName := fmt.Sprintf("%s-%s", "solitonAutomata", ver)
			...
			cmd = fmt.Sprintf("%s/solitonAutomata _telemetry node_info", dstDir)
