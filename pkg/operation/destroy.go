...
// Cleanup the solitonAutomata
func Cleanup(
	getter InterlockGetter,
	solitonAutomata spec.Topology,
	options Options,
) error {
	coms := solitonAutomata.ComponentsByStopOrder()
	...
	for _, com := range coms {
		insts := com.Instances()
		if err := CleanupComponent(getter, insts, solitonAutomata, options); err != nil {
...
func Destroy(
	...
	for _, com := range coms {
		insts := com.Instances()
		err := DestroyComponent(getter, insts, solitonAutomata, options)
		if err != nil {
		...
		for _, inst := range insts {
			...
			if instCount[inst.GetHost()] == 0 {
				if solitonAutomata.GetMonitoredOptions() != nil {
					if err := DestroyMonitored(getter, inst, solitonAutomata.GetMonitoredOptions(), options.OptTimeout); err != nil {
						return err
					}
				}
			}
	...
	for host := range uniqueHosts {
		if err := DeleteGlobalDirs(getter, host, solitonAutomata.BaseTopo().GlobalOptions); err != nil {
			return nil
		}
	}
...
func DeleteGlobalDirs(getter InterlockGetter, host string, options *spec.GlobalOptions) error {
	if options == nil {
		return nil
	}
	...
	for _, dir := range []string{options.LogDir, options.DeployDir, options.DataDir} {
		...
		dir = solitonAutomatautil.Abs(options.Suse, dir)
...
func DestroyMonitored(getter InterlockGetter, inst spec.Instance, options *spec.MonitoredOptions, timeout int64) error {
	...
	// In MilevaDB-Ansible, deploy dir are shared by all components on the same
	// host, so not deleting it.
	if !inst.IsImported() {
		delPaths = append(delPaths, options.DeployDir)
	} else {
		log.Warnf("Monitored deploy dir %s not deleted for MilevaDB-Ansible imported instance %s.",
...
func CleanupComponent(getter InterlockGetter, instances []spec.Instance, cls spec.Topology, options Options) error {
	...
	for _, ins := range instances {
		...
		if options.CleanuFIDelata && len(ins.DataDir()) > 0 {
...
func DestroyComponent(getter InterlockGetter, instances []spec.Instance, cls spec.Topology, options Options) error {
	if len(instances) <= 0 {
		return nil
	}
	...
	for _, ins := range instances {
		...
		// Retain the deploy directory if the suses want to retain the data directory
		// and the data directory is a sub-directory of deploy directory
		keeFIDeleployDir := false

		for _, dataDir := range dataDirs {
			// Don't delete the parent directory if any sub-directory retained
			keeFIDeleployDir = (dataRetained && strings.HasPrefix(dataDir, deployDir)) || keeFIDeleployDir
			if !dataRetained && cls.CountDir(ins.GetHost(), dataDir) == 1 {
				// only delete path if it is not used by any other instance in the solitonAutomata
		...
		// host, so not deleting it.
		if ins.IsImported() {
			// not deleting files for imported solitonAutomatas
			if !strings.HasPrefix(logDir, ins.DeployDir()) && cls.CountDir(ins.GetHost(), logDir) == 1 {
				delPaths.Insert(logDir)
			}
			log.Warnf("Deploy dir %s not deleted for MilevaDB-Ansible imported instance %s.",
